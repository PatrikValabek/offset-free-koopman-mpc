...
def load():
    global A, B, C, loaded_setup, nz, nu, ny, nd, T_real, A_block, A_transformed, A_backtransformed, problem, scaler, scalerU, y_start, y_start_ns, reference, y_setpoint, u_previous, u_previous_ns, P0, Q, R, A_, B_, C_, KF, target_estimation, target_energy_estimation, mpc, Bd, Cd, u_sp
    matrix_C = True
...
# Initial state estimate includes disturbance
    ...
    target_estimation = helper.TargetEstimation(
        A, B, C, loaded_setup['Qy'], loaded_setup['Qu'] * 0, Bd, Cd
    )
    mpc = helper.MPC(A, B, C, loaded_setup['Qy'], loaded_setup['Qu'], loaded_setup['Qdu'], Bd, Cd)

def tests():
    x_f = np.asarray(KF.x).flatten()
    z_s, y_s, u_s = target_estimation.get_target(x_f[nz:], y_setpoint, u_sp)
    z_ref = z_s
    print(z_ref)
    _ = mpc.get_u_optimal(x_f[:nz], x_f[nz:], u_s, u_previous, z_ref)
...
def next_optimal_input(previous_input, measurement, step):
    ...
    _ = KF.step(u_prev, y_now)
    x_f = np.asarray(KF.x).flatten()
    print(x_f[nz:])
    y_setpoint = reference[:, step]
    z_s, y_s, u_s = target_estimation.get_target(x_f[nz:], y_setpoint, u_sp)
    z_ref = z_s
    u_opt = mpc.get_u_optimal(x_f[:nz], x_f[nz:], u_s, u_prev, z_ref)
