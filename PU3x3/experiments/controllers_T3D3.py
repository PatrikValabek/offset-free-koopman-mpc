...
def load():
    global A, B, C, loaded_setup, nz, nu, ny, nd, T_real, A_block, A_transformed, A_backtransformed, problem, scaler, scalerU, y_start, y_start_ns, reference, y_setpoint, u_previous, u_previous_ns, P0, Q, R, A_, B_, C_, TVKF, target_estimation, mpc, Qy, Bd, Cd, u_sp, J_lin
    matrix_C = False
...
# Initial state estimate includes disturbance
    ...
    J_lin = helper.evaluate_jacobian(
        problem.nodes[4],
...
def tests():
    ...
    z_s, y_s, u_s = target_estimation.get_target(
        xf[nz:], y_setpoint, u_sp, get_y(T_real @ xf[:nz]), xf[:nz], J_lin
    )
    z_ref = z_s
    print(z_ref)
    Qz = J_lin.T @ Qy @ J_lin
...
def next_optimal_input(previous_input, measurement, step):
    ...
    z_flat = np.asarray(TVKF.x).flatten()
    z_state = z_flat[:nz]
    ...
    J_kf = helper.evaluate_jacobian(
        problem.nodes[4],
        torch.from_numpy(T_real @ z_state).float().flatten(),
    ) @ T_real
    C_ = np.hstack([J_kf, Cd])
    _ = TVKF.step(
        u_opt,
        y_now,
        get_y(T_real @ z_state),
