#!/usr/bin/env python3
"""
T2D4: Taylor-based MPC with block-diagonal A transformation.
T2 = Target estimation linearizes at previous target
D4 = MPC linearizes at current target
"""
...
def main() -> None:
    ...
    u_opt = mpc.get_u_optimal(
        z_est_[0, :nz], z_est_[:, nz:], u_previous, z_ref, get_y(T_real @ z_s), z_s, J, J_ref, get_y(T_real @ z_s), z_s
    )
    print(u_opt)
    ...
    for i in range(min(8, ny)):
        ...
        if i in (0,1):
            plt.legend()
        if i == 1:
            plt.title('T2D4: Taylor-based MPC with CSTR plant (states vs ref)')
    plt.tight_layout()
    plt.savefig(os.path.join(figures_dir, 'T2D4_states.png'), dpi=200)
    print(f"Saved states plot to {os.path.join(figures_dir, 'T2D4_states.png')}")
    ...
    plt.savefig(os.path.join(figures_dir, 'T2D4_inputs.png'), dpi=200)
    print(f"Saved inputs plot to {os.path.join(figures_dir, 'T2D4_inputs.png')}")
