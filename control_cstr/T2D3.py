#!/usr/bin/env python3
"""
T2D3: Taylor-based MPC with block-diagonal A transformation.
...
def main() -> None:
    ...
    for i in range(min(8, ny)):
        ...
        if i in (0,1):
            plt.legend()
        if i == 1:
            plt.title('T2D3: Taylor-based MPC with CSTR plant (states vs ref)')
    plt.tight_layout()
    plt.savefig(os.path.join(figures_dir, 'T2D3_states.png'), dpi=200)
    print(f"Saved states plot to {os.path.join(figures_dir, 'T2D3_states.png')}")
    ...
    print(f"Saved inputs plot to {os.path.join(figures_dir, 'T2D3_inputs.png')}")
