#!/usr/bin/env python3
"""
T2D2: Taylor-based MPC with block-diagonal A transformation.
Migrated from T2D2.ipynb notebook.
"""
...
def main() -> None:
    ...
    P0 = loaded_setup['P0']
    Q = loaded_setup['Q']  # process noise (already includes Qd block)
    R = loaded_setup['R']  # measurement noise

    # Output disturbance formulation (Bd=0, Cd=I): d acts additively on y.
    Cd = np.eye(ny)
    Bd = np.zeros((nz, nd))
    ...
    # Initial linearization (used both for first TVKF C_ and first target/MPC solve).
    ...
    # TVKF replaces the old EKF here. C_ (=[J, Cd]) is refreshed every step.
    ...
    for k in range(sim_time):
        ...
        # D2: MPC uses the same linearization point (previous target) — Qz is a parameter,
        # so no rebuild needed; just update values passed to get_u_optimal.
        Qz = J.T @ Qy @ J
        Qz_psd = Qz + 1e-8 * np.eye(Qz.shape[0])
    ...
    for i in range(min(8, ny)):
        ...
        if i in (0,1):
            plt.legend()
        if i == 1:
            plt.title('T2D2: Taylor-based MPC with CSTR plant (states vs ref)')
    plt.tight_layout()
    plt.savefig(os.path.join(figures_dir, 'T2D2_states.png'), dpi=200)
    print(f"Saved states plot to {os.path.join(figures_dir, 'T2D2_states.png')}")
    ...
    print(f"Saved inputs plot to {os.path.join(figures_dir, 'T2D2_inputs.png')}")
