...
def tests():
    ...
    z_ref_prev = z_s_energy
    print(z_ref_prev)
    Qz = J.T @ Qy @ J
    Qz_psd = Qz + 1e-8 * np.eye(Qz.shape[0])   
    mpc.build_problem()
    _ = mpc.get_u_optimal(TVKF.x[0, :nz], TVKF.x[:, nz:], u_previous, z_ref_prev, u_s_energy, get_y(T_real @ z_s_energy), z_s_energy, J, Qz_psd)
    # Compute J at current energy target for next TVKF step
    J_prev = helper.evaluate_jacobian(
        problem.nodes[4],
        torch.from_numpy(T_real @ z_ref_prev).float(),
    ) @ T_real
...
def next_optimal_input(previous_input, measurement, step):
    ...
    # TVKF step - linearize at previous energy target
    C_ = np.hstack([J_prev, Cd])
    z_sim = TVKF.step(
        u_prev, y_now,
        get_y(T_real @ z_ref_prev), z_ref_prev,
        J_prev, C_
    ).flatten()

    y_setpoint = reference[:, step]

    # Target/MPC - linearize at previous energy target (J_prev)
    ...
    # Update: J at current energy target for next TVKF step
    z_ref_prev = z_ref
    J_prev = helper.evaluate_jacobian(
        problem.nodes[4],
        torch.from_numpy(T_real @ z_ref_prev).float(),
