...
def load():
    global A, B, C, loaded_setup, nz, nu, ny, nd, T_real, A_block, A_transformed, A_backtransformed, problem, scaler, scalerU, y_start, y_start_ns, reference, y_setpoint, u_previous, u_previous_ns, P0, Q, R, A_, B_, C_, TVKF, target_estimation, target_energy_estimation, mpc, Qy, Bd, Cd, u_sp
    matrix_C = False
...
def tests():
    ...
    J = helper.evaluate_jacobian(
        problem.nodes[4],
        torch.from_numpy(T_real @ TVKF.x[:, :nz].T).float().flatten(),
    ) @ T_real
    z_s, y_s, u_s = target_estimation.get_target(TVKF.x[:, nz:], y_setpoint, u_sp, get_y(T_real @ TVKF.x[0, :nz]), TVKF.x[0, :nz], J)
    z_s_energy, y_s_energy, u_s_energy = target_energy_estimation.get_target(TVKF.x[:, nz:], y_s, u_s, get_y(T_real @ TVKF.x[0, :nz]), TVKF.x[0, :nz], J)
    z_ref = z_s_energy
    print(z_ref)
    Qz = J.T @ Qy @ J
    Qz_psd = Qz + 1e-8 * np.eye(Qz.shape[0])   
    mpc.build_problem()
    _ = mpc.get_u_optimal(TVKF.x[0, :nz], TVKF.x[:, nz:], u_previous, z_ref, u_s_energy, get_y(T_real @ TVKF.x[0, :nz]), TVKF.x[0, :nz], J, Qz_psd)
    # Store J at initial state for first TVKF step
    J_prev = J
...
def next_optimal_input(previous_input, measurement, step):
    ...
    # Get current state before TVKF step (linearization point for TVKF)
    x_current = TVKF.x.flatten()
    z_current = x_current[:nz]
    if step > 0:
        # TVKF step - linearize at current state with J_prev
        C_ = np.hstack([J_prev, Cd])
        z_sim = TVKF.step(
            u_prev, y_now,
            get_y(T_real @ z_current), z_current,
            J_prev, C_
        ).flatten()
    else:
        z_sim = x_current
    ...
    # Target/MPC - linearize at current state estimate
    z_s, y_s, u_s = target_estimation.get_target(
        z_sim[nz:], 
        y_setpoint, 
        u_sp,
        get_y(T_real @ z_current), 
        z_current, 
        J_prev
    )
    ...
    # Compute J at new state estimate for target/MPC
    J = helper.evaluate_jacobian(
        problem.nodes[4],
        torch.from_numpy(T_real @ z_sim[:nz]).float().flatten(),
    ) @ T_real
    # Store J at current state for next TVKF step
    J_prev = J
